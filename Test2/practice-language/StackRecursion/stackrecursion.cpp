...
List language(string input_str, int k)
{
    ...
    while(!s.empty())
    {
        ...
        {
            if(results.find(curr.fixed_str) == NULL)
                {
                    results.insert(curr.fixed_str); // insert parent
                    // create children
                    for(int i = 0; i < input_str.length(); i++)
                    {
                        s.push({input_str, curr.fixed_str + input_str[i]});
                    }
                }

        }
        else if(results.find(curr.fixed_str) == NULL) results.insert(curr.fixed_str);
    }
