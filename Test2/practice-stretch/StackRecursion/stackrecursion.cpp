...
List stretch(string input_str, int k)
{
    ...
    while(!s.empty())
    {
        ...
        newIn = curr.in_str;
        newFix = curr.fixed_str;
        if(curr.in_str.empty() && results.find(curr.fixed_str) == NULL)
        {
        ...
        else
        {
            ...
            {
                newIn = curr.in_str.substr(i+1);
                // cout << "Current Pair: " << curr.in_str << ", " << curr.fixed_str << endl;
                // cout << "New: " << newIn << endl;
    
                for(int j = 0; j < k; j++)
                {
                    // cout << "Pushing: " << newIn << ", " << newFix + curr.in_str[i] << endl;
                    s.push({newIn, newFix += curr.in_str[i]});
                }
            }
            // cout << endl;
        }
