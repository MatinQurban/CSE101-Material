...
void BinHeap :: extractMax()
{
    ...
    while(childIdx <= size)
    {
        if(childIdx+1 <= size)
        {
            childIdx = A[childIdx] > A[childIdx+1] ? childIdx : childIdx+1;
        }
    ...
    if(size < 1) // heap is empty, nothing to extract
        return;
    A[1] = A[size]; // swap max with last element
    size--; // decrement size, so max is now "deleted"
    //sink(1); // call sink at the root to fix the heap property
    return;
} 
/******************************************************/
/******************************************************/
